  ...
  function doInstall(installation, nextInstall) {
    ...
    function branchInstall(branch, nextBranch) {
      ...
      installation.install(branch.url, function(err) {
        var pkg_json = path.join(process.cwd(),
                                 installation.destination,
                                 'package.json')
          , pkg
        if (!err) {
          pkg = exports.JSONReader(pkg_json)
          installation.installed = { package: pkg, branch: branch }
          exports.logger.info('done. (' + installation.name + '@' + pkg.version
                                        + ' from ' + branch.name + ')')
        }
        nextBranch()
      })
    ...
    _.each(installations, function(installation) {
      if (installation.installed) {
        exports.logger.info('Installed ' +
                    installation.name + '@' + installation.installed.package.version
