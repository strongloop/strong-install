...
function cmdInstall(pkgName, branches, opts) {
  ...
  if (pkgName && pkgName != '.') {
    ...
    exports.logger.info('No package specified, looking for dependencies in ' + pkgJSON)
...
function installList(pkgs, branches, repo, destination) {
  exports.logger.info('Installing ' + pkgs.join(', '))
