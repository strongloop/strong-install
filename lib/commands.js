...
function cmdInstall(pkgName, branches, opts, done) {
  ...
  if (pkgName && pkgName != '.') {
    ...
    packages = _.keys(pkgJSON.dependencies || {}).concat(_.keys(pkgJSON.devDependencies || {}))
  }
...
function installList(pkgs, branches, opts, doneInstallList) {
  ...
  var repo = opts.repo
    ...
    , installations = _.map(pkgs, installationTaskFromName)

  async.each(installations, doInstall, report)

  function installationTaskFromName(name) {
