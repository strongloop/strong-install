...
function installList(pkgs, branches, opts, doneInstallList) {
  ...
  var repo = opts.repo
    , installations = _.map(pkgs, function(name) {
        return installationTaskFromName(name, opts.destination)
      })
    , installationLog = []
  ...
  function installationTaskFromName(name, destination) {
    ...
    function installSubDeps(cb) {
      var pkgJSONPath = path.join(process.cwd(), installer.destination,
                                  'package.json')
        , pkgs = packagesFromPackageJSON(pkgJSONPath, false)
        , subDestination = path.join(installer.destination, 'node_modules')
        , subDeps = _.map(pkgs, function(name) {
            return installationTaskFromName(name, subDestination)
          })
      async.each(subDeps, doInstall, function(err) {
        ...
        if (opts.npm) {
          exports.logger.info('Queued npm install ', installer.destination,
                              destination)
          setImmediate(npmInstall, cb)
        } else {
          setImmediate(cb, err)
