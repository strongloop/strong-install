...
function commitAndParents(head, callback) {
  git.gitRevList('--parents -n 1 ' + head, parseCommits)
...
function refsFor(sha1, callback) {
  var refsFilter = grep(new RegExp(sha1), _.partialRight(parseRefs, callback))
...
function forks(start, callback) {
  ...
  gitRevList('--children ' + start, function(err, revs) {
...
function branchesContaining(commit, callback) {
  var error = callback || console.log
    , success = _.partial(error, null)
  commit = commit || 'HEAD'
  gitBranch('--all --contains ' + commit, function(err, branches) {
    if (err) return error(err)
    _(branches)
      ...
      if (err)
        ...
        async.sortBy(_(branches).flatten().uniq().value(),
                    function(br, callback) {
                      gitLog('-1 --pretty=format:%at ' + br, function(err, timestamp) {
                        callback(err, err ? null : -parseInt(timestamp))
                      })
                    }, error)
        // _(branches).flatten().uniq().tap(success)
    })
  })
}
