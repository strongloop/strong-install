...
function refsMap(callback) {
  ...
  git.gitShowRef(function refsFilter(err, lines) {
    var isRefHead = hasPrefix('refs/heads/')
      , stripRefHead = stripPrefix('refs/heads/')
      , isRemoteRef = hasPrefix('refs/remotes/origin/')
    ...
    if (err) {
      ...
      _(lines)
        ...
        .each(function(line) {
          ...
          if (isRefHead(ref)) {
            //cache.refsMap[sha1].push(stripRefHead(ref))
          } else if (isRemoteRef(ref)) {
            cache.refsMap[sha1].push(stripRemoteRef(ref))
          } else {
...
function refsFor(sha1, callback) {
  var sha1Regex = new RegExp(sha1)
  git.gitShowRef(function refsFilter(err, lines) {
    if (err) return callback(err)
    var refs = _(lines)
      .select(matches(sha1Regex))
      .invoke('trim')
      .map(function(line) {
...
var git = {
  ...
  gitRevList: function gitRevList(args, callback) {
    if (cache.gitRevList) return process.nextTick(function() { callback(null, cache.gitRevList) })
    exec('git rev-list ' + args, function(err, stdout, stderr) {
      if (err) {
        cache.gitRevList = null
      } else {
        cache.gitRevList = _.map(lines(stdout), toWords)
      }
      callback(err, cache.gitRevList)
    })
  },
